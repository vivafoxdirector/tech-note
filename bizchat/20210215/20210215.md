...
# 회의
1. Admin > 서비스관리 > 상품변경이력 > 상품항목: 일반 으로 나옴 이게 맞는것인가?
...
3. [] 레드마인 대응
	...
	- [X] #2538
	- [-] #2534 => 앤젤라 대리와 상의 화면이 어디로 이동이 되어야 하는가?(MT/MO 마찬가지)
	- [] #2528
		- [X] 공지사항
		- [X] FAQ
		- [X] 다운로드 사업별로 조회되어야 한다.
		- [] 이벤트 조회 => 테이블 추가해야 할듯하다
	- [] #2553
		- 사업자Admin 으로 로그인하면 '요청파일 다운로드함'이라는 메뉴가 생겨야 한다. 내부의 조회도 사업별로 나와야 한다.

4. [] 발송 백엔드 개발 오프라인 회의 
	- 오비즈 발송 개발
	- 오비즈 상품코드 관련 연관성 개발

3. [] 딜러사 > 고객사 조회 관련 이슈 (원복함)
	...
	````
4. [] 어드민 로그인 없이 사용자 등록(이경우는 사업을 선택하도록 하거나 해야함)
	- eno_01
5. [] 유찬석 매니저와 통화내용중 확인사항
